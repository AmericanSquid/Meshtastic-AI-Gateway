...
async def _restart_process(router: MessageRouter) -> None:
    await asyncio.sleep(2.0)
    if router.hermes is not None:
        await router.hermes.stop()
    os._exit(75)
...
async def handle_mesh_command(
    ...
    if command == "!health":
        ...
        if router.hermes_selected:
            state = "running" if router.hermes is not None and router.hermes.running else "stopped"
            return f"Gateway OK | Mesh {mesh['status']} | Mode hermes | Hermes {state}"
        active = ai.get("active") or "idle"
        if router.hermes is None:
            hermes = "disabled"
        else:
            hermes = "running" if router.hermes.running else "ready"
        return (
    ...
    if command == "!new":
        ...
        if router.hermes_selected and router.hermes is not None:
            await router.hermes.reset()
            return "New Hermes chat."
        return "New chat."
    ...
    if command == "!hermes":
        ...
        try:
            await router.hermes.start()
        except Exception as exc:
            return f"Hermes failed to start: {type(exc).__name__}: {str(exc)[:120]}"
    ...
    if command == "!restart":
        asyncio.create_task(_restart_process(router))
